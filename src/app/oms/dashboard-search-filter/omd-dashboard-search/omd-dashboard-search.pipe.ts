...
@Pipe({
  name: 'omdDashboardSearch',
})
export class OmdDashboardSearchPipe implements PipeTransform {
  ...
  transform(items: any[], searchTextDashboard: string): any[] {
    ...
    this.sharedService.updateFilteredDataLength(filteredItems?.length, 'cxo');
    this.sharedService.cxoDashData = filteredItems;
