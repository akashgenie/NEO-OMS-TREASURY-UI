...
export class ReconsallationGuard implements CanActivate {
  constructor(
  ...
  canActivate(
    ...
    if (
      (authUser?.token && this.giveAccess?.bonddashboard) ||
      this.giveAccess?.dealerdashboard ||
      this.giveAccess?.opsdashboard
    ) {
      return true;
    } else {
